 ...
 */
class SalonGallery extends Model implements ModelInterface{
    ...
    public function saveObject(SalonDTO $salonDTO)
    {
        $salongallery= new SalonGallery();
        //Delete old one.
        SalonGallery::where('user_phone_no', $salonDTO->getUserPhoneNo())->delete();

        $salongallery->user_phone_no = $salonDTO->getUserPhoneNo();
        $salongallery->logo = $salonDTO->getSalonLogo();
        $salongallery->gallery= $salonDTO->getSalonGallery();

        $obj = $salongallery->save();
