 ...
 */
class PaymentPayeeModel extends Model implements ModelInterface{
    ...
    public function saveObject(PaymentPayeeDTO $paymentPayeeDTO)
    {
        ...
        $this->user_id = $paymentPayeeDTO->getUserId();

        $obj = $this->save();
        $paymentPayeeDTO->setId($this->id);
    ...
    {
        $targetPaymentPayeeDTO = new PaymentPayeeDTO();
        $payeeArr = PaymentPayeeModel::where('id', $id )->get();
        if($payeeArr)
        {
            $targetPaymentPayeeDTO->setId($payeeArr[0]->id);
            $targetPaymentPayeeDTO->setPayeeName($payeeArr[0]->payee_name);
            $targetPaymentPayeeDTO->setUserId($payeeArr[0]->user_id);
            return $targetPaymentPayeeDTO;
        }
        return null;
    }
    ...
    protected $table = 'isg_payment_payee';
