 ...
 */
class PaymentUserInvoiceModel extends Model {
    ...
    public function saveObject(PaymentUserInvoiceDTO $userInvoiceDTO)
    {
        $this->currency_code = $userInvoiceDTO->getCurrencyCode();
        $this->payee_amount = $userInvoiceDTO->getPayeeAmount();
        $this->payment_payee_id = $userInvoiceDTO->getPaymentPayeeId();
        $this->invoice_id = $userInvoiceDTO->getInvoiceId();

        //Save Payee Name
        $bPaymentPayee = new BPaymentPayee();
        $paymentPayeeDTO = $bPaymentPayee->getDTOById($userInvoiceDTO->getPaymentPayeeId());
        $this->payee_name = $paymentPayeeDTO->getPayeeName();

        //Save User Name
        $bInvoice = new BInvoice();
        $invoiceDTO = $bInvoice->getDTOById($userInvoiceDTO->getInvoiceId());

        $bUser = new BUser();
        $this->invoice_user_name = $bUser->getDTOById($invoiceDTO->getUserId())->getFirstName();

        $obj = $this->save();
        $userInvoiceDTO->setId($this->id);
    ...
    {
        $query = "SELECT invoice_amount as 'amount' FROM isg_invoice WHERE id = ". $userInvoiceDTO->getInvoiceId();
        $result = DBUtil::select($query);
        if($result)
        {
    ...
    protected $table = 'isg_payment_user_invoice';
