...
use App\Models\api\v1\paymentmodel\PaymentVendorMasterModel;
...
class BPaymentVendorMaster {
    ...
    public function saveObject(PaymentVendorMasterDTO $paymentVendorMasterDTO) {
        $paymentVendorMasterModel = new PaymentVendorMasterModel();
        $obj = $paymentVendorMasterModel->saveObject($paymentVendorMasterDTO);

        if ($paymentVendorMasterDTO->getApiCall() == AppDTO::$TRUE_AS_STRING) {
            $response['Status'] = APICodes::$TRANSACTION_SUCCESS;
            // $response['Message'] = "Successfully Saved Object";
            $response['Message'] = "Payment Id " . $obj->getId();
