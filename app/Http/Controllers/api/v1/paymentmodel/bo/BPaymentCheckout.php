...
use App\Models\api\v1\paymentmodel\PaymentCheckoutModel;
...
class BPaymentCheckout {
    ...
    public function saveObject(PaymentCheckoutDTO $paymentCheckoutDTO) {
        $paymentCheckoutModel = new PaymentCheckoutModel();
        $paymentCheckoutModel->saveObject($paymentCheckoutDTO);

        if ($paymentCheckoutDTO->getApiCall() == AppDTO::$TRUE_AS_STRING) {
