...
class BInvoice implements BusinessInterface {
    ...
    public function saveObject(InvoiceDTO $invoiceDTO) {
        ...
        $result = $invoiceModel->saveObject($invoiceDTO);

        if ($result == AppDTO::$FALSE_AS_STRING) {
            if ($invoiceDTO->getApiCall() == AppDTO::$TRUE_AS_STRING) {
        ...
        if ($result == AppDTO::$FALSE_AS_STRING) {
            ...
            AppDTO::$TRUE_AS_STRING;
        }
    }

    public function getDTOById($id) {
        $invoiceModel = new InvoiceModel();
        return $invoiceModel->getDTOById($id);
    }
